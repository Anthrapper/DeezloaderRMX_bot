...
downloa = deezloader.Login(settings.username, settings.password)
...
def generate_token():
    credentials = oauth2.SpotifyClientCredentials(
        client_id="4fe3fecfe5334023a1472516cc99d805", client_secret="0f02b7c483c04257984695007a4a8d5c")
        ...
        if "spotify" in music:
            if "track/" in music:
                ...
                for a in range(b // 50):
                    ...
                    for a in tracks['items']:
                        links.append(a['external_urls']['spotify'])
                msg = telepot.message_identifier(bot.sendMessage(
                    chat_id, ("About " + str((b * 13) // 60)) + " minutes"))
                ...
                musi = music.split("/")
                try:
                    tracks = spo.user_playlist_tracks(
                        musi[-3], playlist_id=musi[-1])
                except Exception as a:
                    if not "The access token expired" in str(a):
                        bot.sendMessage(chat_id, "Invalid link ;)")
                        delete(chat_id)
                ...
                for a in range(tracks['total'] // 100):
                    ...
                    for a in tracks['items']:
                        ...
                        links.append(a['track']['external_urls']['spotify'])
                bot.sendMessage(
                    chat_id, ("About " + str((tracks['total'] * 13) // 60)) + " minutes")
            ...
            if "track/" in music:
                ...
                url = json.loads(requests.get(
                    "http://api.deezer.com/track/" + music.split("/")[-1]).text)
                try:
                    if "error" in str(url):
                        bot.sendMessage(chat_id, "Invalid link ;)")
                        delete(chat_id)
                ...
                try:
                    image.append(
                        url['album']['cover_xl'].replace("1000", "90"))
                except:
                ...
                if "?" in music:
                    music, a = music.split("?")
                url = json.loads(requests.get(
                    "http://api.deezer.com/album/" + music.split("/")[-1]).text)
                try:
                    if "error" in str(url):
                        bot.sendMessage(chat_id, "Invalid link ;)")
                        delete(chat_id)
                ...
                if "?" in music:
                    music, a = music.split("?")
                url = json.loads(requests.get(
                    "https://api.deezer.com/playlist/" + music.split("/")[-1] + "/tracks").text)
                try:
                    if "error" in str(url):
                        bot.sendMessage(chat_id, "Invalid link ;)")
                        delete(chat_id)
                ...
                times = 0
                for a in url['data']:
                    image.append(json.loads(requests.get("http://api.deezer.com/track/" +
                                                         str(a['id'])).text)['album']['cover_xl'].replace("1000", "90"))
                    times += 1
                bot.sendMessage(
                    chat_id, ("About " + str((times * 13) // 60)) + " minutes")
    ...
    try:
        ...
        for a in range(len(z)):
            try:
                ...
                bot.sendMessage(chat_id, "Error downloading " +
                                z.split("/")[-1] + " :(")
    except UnboundLocalError:
        bot.sendMessage(chat_id, "Invalid link ;)")
...
def Audio(audio, chat_id):
    ...
    if audio['status']['msg'] != "Success":
        bot.sendMessage(
            chat_id, "Sorry cannot detect the song from audio :(, retry...")
    else:
        ...
        url = json.loads(requests.get("https://api.deezer.com/search/track/?q=" +
                                      track.replace("#", "") + " + " + artist.replace("#", "")).text)
        try:
            ...
            try:
                url = json.loads(requests.get("https://api.deezer.com/search/track/?q=" +
                                              track.replace("#", "").split(" ")[0] + " + " + artist.replace("#", "")).text)
                for a in range(url['total'] + 1):
                ...
                try:
                    ...
                    None
                try:
                    id = "https://www.deezer.com/track/" + \
                        str(audio['metadata']['music'][0]
                            ['external_metadata']['deezer']['track']['id'])
                    url = json.loads(requests.get(
                        "http://api.deezer.com/track/" + id.split("/")[-1]).text)
    ...
    if query_data == "Infos with too many bytes" or query_data == "No informations":
        ...
        if ans != "1":
            try:
                if users[from_id] == 2:
                    bot.answerCallbackQuery(
                        query_id, "Wait to finish and repress download, did you thought that you could download how much songs did you want? :)", show_alert=True)
                    return
                else:
                    users[from_id] += 1
...
def start1(msg):
    ...
    if content_type == "text" and msg['text'] == "/start":
        ...
        bot.sendMessage(
            chat_id, "This bot has been created for download songs by a Spotify or Deezer link, by an artist and song name or by an audio or voice message")
        bot.sendPhoto(chat_id, open("example.jpg", "rb"),
                      caption="The bot commands can find here")
    elif content_type == "text" and msg['text'] == "/link":
        bot.sendMessage(
            chat_id, "Insert the link to download the music or album from spotify or deezer")
        stage[chat_id] = 1
        ...
        song = msg['text']
        threading.Thread(target=Name1, args=(
            artist[chat_id], song, chat_id)).start()
...
def start2(msg):
    ...
    if content_type == "text" and msg['text'] == "/start":
        ...
        try:
            if users[chat_id] == 2:
                bot.sendMessage(chat_id, "Wait to finish and resend the " + content_type +
                                ", did you thought that you could download how much songs did you want? :)")
            else:
                users[chat_id] += 1
                threading.Thread(target=Link1, args=(music, chat_id)).start()
        except KeyError:
